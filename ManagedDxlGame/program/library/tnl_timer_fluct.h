...
*/
	...
	private:
		...
		inline bool seqAccel(const float deltatime) {
			...
			float _stage_id = sequence_.getProgressTime();
			T vn = to_complete_normalize_;
			*origin_ = start_ + vn * ( ( a * powf( _stage_id, 2 ) ) * 0.5f ) ;
			return false;
		}
		...
		inline bool seqBrake(const float deltatime) {
			...
			float _stage_id = sequence_.getProgressTime();
			T vn = to_complete_normalize_;
			*origin_ = complete_ - vn * ((a * powf(complete_time_ - _stage_id, 2)) * 0.5f);
			return false;
		}
		...
		inline bool seqSin(const float deltatime) {
			...
			float _stage_id = sequence_.getProgressTime() / complete_time_ ;
			float s = sin( ToRadian(_stage_id * 180.0f) ) ;
		...
		inline bool seqCos(const float deltatime) {
			...
			T v = static_cast<T>(complete_ - start_);
			float _stage_id = sequence_.getProgressTime() / complete_time_;
			float c = cos(ToRadian(_stage_id * 180.0f));
